...
function rankingRecordsToPlayers(records: { name: string; position: string; team: string; sourceRank: number; byeWeek?: number | null }[], sourceName: string): Player[] {
...
export default function Home() {
  ...
  const [expandedRosterTeams, setExpandedRosterTeams] = useState<number[]>([4]);
  ...
  const [draftCenterView, setDraftCenterView] = useState<"rosters" | "board" | "tiers">("rosters");
  const [sidePanelMode, setSidePanelMode] = useState<"none" | "signals" | "watchlist" | "roster">("signals");
  ...
  useEffect(() => {
    if (activeDraftId) setExpandedRosterTeams([mySlot]);
  }, [activeDraftId]);

  useEffect(() => setPosition("ALL"), [rosterPositions]);
  ...
  const positionFilters = ["ALL", ...Array.from(new Set(rosterPositions.filter((slot) => slot !== "BN").map((slot) => slot === "SUPER_FLEX" ? "SFLEX" : slot === "DST" ? "D/ST" : slot)))];
  const filtered = available.filter((player) => (position === "ALL" || player.position === position || (position === "D/ST" && ["DST", "DEF"].includes(player.position)) || (position === "FLEX" && ["RB", "WR", "TE"].includes(player.position)) || (position === "SFLEX" && ["QB", "RB", "WR", "TE"].includes(player.position))) && player.name.toLowerCase().includes(search.toLowerCase())).sort((a, b) => a.rank - b.rank);
  ...
  const toggleRosterTeam = (team: number) => setExpandedRosterTeams((current) => current.includes(team) ? current.filter((item) => item !== team) : [...current, team]);
    ...
    <main className="app-shell draft-shell">
      ...
      <section className={`draft-grid draft-view-${draftCenterView}`}>
        <aside className={`recommend-panel side-panel-${sidePanelMode}`}><div className="panel-title player-list-title"><div><span className="eyebrow">YOUR RANKINGS</span><h2>Available players</h2></div><div className="side-panel-toggle" role="group" aria-label="Player list companion panel">{(["none", "signals", "watchlist", "roster"] as const).map((mode) => <button className={sidePanelMode === mode ? "active" : ""} aria-pressed={sidePanelMode === mode} onClick={() => setSidePanelMode(mode)} key={mode}>{mode === "none" ? "None" : mode === "signals" ? "Signals" : mode === "roster" ? "Roster" : `Watchlist${watchlist.length ? ` ${watchlist.length}` : ""}`}</button>)}</div></div>
          ...
          {sidePanelMode === "watchlist" && <section className="draft-watchlist" aria-label="Player watchlist"><div className="draft-signals-heading"><strong>Watchlist</strong><small>{watchlist.length} available</small></div>{watchlist.length ? <div className="watchlist-players">{watchlist.map((player) => <div className="watchlist-player" key={player.id}><div className="watchlist-draft"><b>{player.rank}</b><span><strong>{player.name}</strong><small>{player.position} · {player.team}{player.byeWeek ? ` · Bye ${player.byeWeek}` : ""}</small></span><button className="draft-plus" aria-label={`Draft ${player.name}`} onClick={() => draftPlayer(player)}>＋</button></div><button className="watch-toggle active" title="Remove from watchlist" aria-label={`Remove ${player.name} from watchlist`} onClick={() => toggleWatchlist(player)}>★</button></div>)}</div> : <div className="watchlist-empty"><span>☆</span><strong>No players watched yet</strong><small>Use the star beside a player to add them.</small></div>}</section>}
          {sidePanelMode === "roster" && <section className="draft-roster-panel" aria-label="Your roster"><div className="draft-signals-heading"><strong>Your roster</strong><small>{myRosterPicks.length} drafted · {rosterSlots.filter((slot) => !slot.player).length} open</small></div><div className="compact-lineup-slots">{rosterSlots.map((slot) => <div className={slot.player ? "compact-lineup-slot filled" : "compact-lineup-slot"} key={slot.id}><span>{slot.position === "SUPER_FLEX" ? "SFLEX" : slot.position}</span><div>{slot.player ? <><strong>{slot.player.name}</strong><small>{slot.player.team} · Pick {slot.player.pick}</small></> : <><strong>Open spot</strong><small>{rosterPositionLabels[slot.position]}</small></>}</div></div>)}</div></section>}
          <div className="filters"><div className="search"><span>⌕</span><input aria-label="Search available players" placeholder="Search players" value={search} onChange={(e) => setSearch(e.target.value)} /></div><div className="filter-row">{positionFilters.map(p => <button className={position === p ? "active" : ""} onClick={() => setPosition(p)} key={p}>{p}</button>)}</div></div>
          <div className="player-list">{filtered.slice(0, 60).map((player) => <div className="player-list-row" key={player.id}><div className="player-draft-action"><b>{player.rank}</b><div><strong>{player.name}</strong><small>{player.team} · Tier {tierLabel(player)}{player.byeWeek ? ` · Bye ${player.byeWeek}` : ` · ${player.source}`}</small>{strategyBadge(player)}</div><span className={`pos ${positionColors[player.position] || ""}`}>{player.position}</span><button className="draft-plus" aria-label={`Draft ${player.name}`} onClick={() => draftPlayer(player)}>＋</button></div><button className={watchlistIds.includes(player.id) ? "watch-toggle active" : "watch-toggle"} title={watchlistIds.includes(player.id) ? "Remove from watchlist" : "Add to watchlist"} aria-label={`${watchlistIds.includes(player.id) ? "Remove" : "Add"} ${player.name} ${watchlistIds.includes(player.id) ? "from" : "to"} watchlist`} onClick={() => toggleWatchlist(player)}>{watchlistIds.includes(player.id) ? "★" : "☆"}</button></div>)}</div>
        </aside>
        <section className={draftCenterView === "tiers" ? "board-panel tier-board-panel" : draftCenterView === "board" ? "board-panel true-draft-board-panel" : "board-panel"}><div className="board-heading"><div><span className="eyebrow">LIVE DRAFT</span><h2>{draftCenterView === "rosters" ? "League rosters" : draftCenterView === "board" ? "Draft board" : "Available by tier"}</h2></div><div className="board-heading-actions">{draftCenterView === "rosters" && <div className="roster-board-controls"><button onClick={() => setExpandedRosterTeams([])}>Collapse all</button><button onClick={() => setExpandedRosterTeams(Array.from({ length: teams }, (_, index) => index + 1))}>Expand all</button></div>}{sleeperDraftId && <div className={`sleeper-board-sync ${sleeperSyncPaused ? "sync-paused" : `sync-${sleeperSyncState}`}`}><button className="sleeper-sync-summary" title={sleeperSyncMessage} onClick={() => { if (unresolvedSleeperPicks.length) setSleeperMatcherOpen(true); }}><i /> <span>{sleeperSyncPaused ? "Manual mode" : sleeperSyncState === "live" ? "Sleeper live" : sleeperSyncState === "error" ? "Sync issue" : "Connecting"}</span>{sleeperUnmatchedCount > 0 && <b>{sleeperUnmatchedCount}</b>}</button>{sleeperSyncPaused ? <button className="manual-switch reconnect" disabled={sleeperSyncState === "connecting"} onClick={reconnectSleeperDraft}>{sleeperSyncState === "connecting" ? "Reconnecting…" : "Reconnect Sleeper"}</button> : <button className="manual-switch" onClick={pauseSleeperSync}>Switch to manual</button>}</div>}<div className="board-view-toggle" role="group" aria-label="Draft board view"><button className={draftCenterView === "rosters" ? "active" : ""} aria-pressed={draftCenterView === "rosters"} onClick={() => setDraftCenterView("rosters")}><span>▦</span> Roster View</button><button className={draftCenterView === "board" ? "active" : ""} aria-pressed={draftCenterView === "board"} onClick={() => setDraftCenterView("board")}><span>▥</span> Draft Board</button><button className={draftCenterView === "tiers" ? "active" : ""} aria-pressed={draftCenterView === "tiers"} onClick={() => setDraftCenterView("tiers")}><span>≡</span> Tier Board</button></div><span>Round {round} of {rosterPositions.length}</span></div></div>
          {draftCenterView === "rosters" ? <div className="roster-board">{Array.from({length: teams}, (_, i) => i + 1).map(team => { const teamPicks = picks.filter((pick) => pick.roster === team); const expanded = expandedRosterTeams.includes(team); return <div className={`${team === mySlot ? "roster my-roster" : "roster"}${expanded ? " expanded" : " collapsed"}`} key={team}><div className="roster-head"><div className="roster-team-name"><button className="roster-expand" aria-expanded={expanded} aria-label={`${expanded ? "Collapse" : "Expand"} ${teamDisplayName(team)} roster`} onClick={() => toggleRosterTeam(team)}><b>{expanded ? "▾" : "▸"}</b><span>{teamDisplayName(team)}</span><small>{teamPicks.length}</small></button>{team !== mySlot && <button aria-label={`Rename ${teamDisplayName(team)}`} title="Rename team" onClick={() => renameTeam(team)}>✎</button>}</div>{team === onClock && <i>ON CLOCK</i>}</div>{expanded && <div className="roster-players">{teamPicks.map(p => <div className="roster-player" key={p.pick}><span className={`pos ${positionColors[p.position] || ""}`}>{p.position}</span><div><strong>{p.name}</strong><small>{p.team} · Pick {p.pick}</small></div></div>)}{teamPicks.length === 0 && <div className="empty-roster">No picks yet</div>}</div>}</div>})}</div> : draftCenterView === "board" ? <div className="true-draft-board">{Array.from({ length: teams }, (_, index) => index + 1).map((team) => <section className={team === mySlot ? "draft-team-column my-draft-column" : "draft-team-column"} key={team}><header><span>{teamDisplayName(team)}</span><small>{picks.filter((pick) => pick.roster === team).length} picks</small></header><div>{picks.filter((pick) => pick.roster === team).map((pick) => <article key={pick.pick}><b>{pick.pick}</b><span className={`pos ${positionColors[pick.position] || ""}`}>{pick.position}</span><div><strong>{pick.name}</strong><small>{pick.team}</small></div></article>)}{picks.every((pick) => pick.roster !== team) && <div className="empty-draft-column">No picks</div>}</div></section>)}</div> : <div className="draft-tier-board">{tierBoardColumns.map((column) => <section className={`draft-tier-column tier-column-${column.position.toLowerCase()}`} key={column.position}><header><strong>{column.position === "QB" ? "QUARTERBACK" : column.position === "RB" ? "RUNNING BACK" : column.position === "WR" ? "WIDE RECEIVER" : "TIGHT END"}</strong><span>{column.groups.reduce((total, group) => total + group.players.length, 0)} left</span></header><div>{column.groups.map((group) => <div className={group.tier === null ? "draft-tier-group na-tier" : "draft-tier-group"} key={group.tier ?? "na"}><div className="draft-tier-label"><span>{group.tier === null ? "N/A" : `TIER ${group.tier}`}</span><i>{group.players.length}</i></div>{group.players.map((player) => <div className="tier-player-row" key={player.id}><button className="tier-draft-action" onClick={() => draftPlayer(player)}><div><strong>{player.name}</strong><small>{player.team} · Rank {player.rank}</small>{strategyBadge(player)}</div><span>＋</span></button><button className={watchlistIds.includes(player.id) ? "watch-toggle active" : "watch-toggle"} title={watchlistIds.includes(player.id) ? "Remove from watchlist" : "Add to watchlist"} aria-label={`${watchlistIds.includes(player.id) ? "Remove" : "Add"} ${player.name} ${watchlistIds.includes(player.id) ? "from" : "to"} watchlist`} onClick={() => toggleWatchlist(player)}>{watchlistIds.includes(player.id) ? "★" : "☆"}</button></div>)}</div>)}</div></section>)}</div>}
        </section>
